...
- 在 pulse/软触发回顾时，模型先 review pool，再决定每一条是删掉、提升成 reminder、还是提升成 memory。
...
2. `pulse`（软触发回顾）：不是「必须说话」的义务，而是「检查一下」的义务。它用于 post-turn audit、位置/语境触发、手动 system trigger 等非到期场景。默认顺序是 review 语境 → 判断要不要发一条短消息 → 不发就做一个小的私下动作 → 做一次跟进决策。即便选择沉默，也优先做一件小事（排提醒、标记习惯、查一条 Obsidian 信号、存记忆等），不能用沉默偷懒。
3. `reminder`（到期义务）：现在就行动的义务。把提醒文本转化成当下最有用的动作，不要假设用户已经做了。

> 命名说明：`pulse` 是模型侧的通用软触发语义；`checkin` 是宿主侧的随机/间隔触达机制与命令名。现在 contact-gap 触达走 `checkin`，不再等同于普通 `pulse`。
...
- **embedding**：memory 的语义检索后端。配置 `CYBERBOSS_EMBEDDING_*` 后，写入时自动算向量，`search` 走余弦相似度；新增记忆时也会做一次近重复检测，命中阈值就把候选旧记忆回传给模型，让模型决定是更新已有条目还是坚持新增。未配置时退回子串匹配。老数据用 `cyberboss_memory_reindex` 补算一次即可。
...
- **队列与触发**：本地提醒队列、system/check-in 触发队列、时间线截图任务队列，加上 title-pool / memory 等内部存储。
...
CYBERBOSS_PULSE_MIN_INTERVAL_MS=10800000   # 随机 life pulse，默认 180-360 分钟
CYBERBOSS_PULSE_MAX_INTERVAL_MS=21600000
CYBERBOSS_CHECKIN_MIN_INTERVAL_MS=300000   # contact-gap poller 的唤醒区间，默认 5-15 分钟
CYBERBOSS_CHECKIN_MAX_INTERVAL_MS=900000
CYBERBOSS_MAX_CONTACT_GAP_MINUTES=45       # 多久没联系后触发 check-in rescue
...
CYBERBOSS_MEMORY_DEDUP_THRESHOLD=0.92
CYBERBOSS_MEMORY_DEDUP_LIMIT=3
```
...
- `/stop` — 停止当前运行中的 turn
- `/checkin <min>-<max>` — 调整当前项目的随机 check-in 唤醒区间
- `/pulse <min>-<max>` — 调整当前项目的随机 life pulse 触达区间
- `/chunk <number>` — 调整微信短回复片段的最小合并大小
- `/activity` — 查看当前绑定工作区/聊天里的 activity 线程
- `/activity items` — 展开查看当前 open activity 里的所有 open item
- `/reminder` — 查看当前绑定工作区/聊天里的 hard reminder
- `/yes` — 本次批准放行
...
- `activity` 在 phase 1 里不再默认自动创建 1:1 check-back reminder。
- `activity` 用于承载同一条工作线程下的多个 item。
- `activity` 默认会自动 arm 第一轮 review；如果不给参数，默认 cadence 是 30-60 分钟。
- `reminder` 保留给有明确时间点的 hard reminder。
...
- `system-message-queue.json` — 系统/软触发队列
- `deferred-system-replies.json` — 等待下一个可用微信 context token 的回复
- `checkin-config.json` — 保存的 check-in 唤醒区间
- `pulse-config.json` — 保存的 scheduled life pulse 区间
...
- **core**：读配置、选 channel/runtime/集成、编排能力而非实现具体协议、own 触发语义（user message / pulse / checkin / reminder / approval）。
