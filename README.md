...
cyberboss-traceflow 把一个本地运行的编程型 Agent（Codex 或 Claude Code）桥接进微信，让它变成一个常驻在线、记得你这一天发生了什么、并且会在合适的时机主动把你拉回来的生活陪伴体。你不用「开始一个会话」——它就活在那个你本来每天都会打开的聊天界面里。

它最初来自 [WenXiaoWendy/Cyberboss](https://github.com/WenXiaoWendy/Cyberboss)，但经过大量改造，已经不再是原来那个偏 ADHD Coach 的系统，而收窄到一件更具体的事。

## 它是为哪一类事准备的

不是「忘了开会」那种——开会有人会来找你，忘了会有后果。

它管的是另一类：**不做也没人会来追究你**的事。

- 椅子上堆了三天的衣服，没有任何后果逼你收，所以你不会收。
- 想种草某个东西，念头一闪而过，三天后百无聊赖时却再也想不起来。
- 洗碗、扔垃圾、回一条不紧急的消息：每件都不难，但就是启动不了；而且同一个提醒喊到第三次，对你已经彻底失效了。

这类事没有硬性的时间点，也没有严重的外部后果去 Push 你去执行——而恰恰是它们，最容易在 ADHD 的生活里一直拖着，直到变成房间里那头谁都假装看不见的大象。

traceflow 做的，是把这类事的管理权从你手里挪走一点，交给一个始终在线、保持记忆、能跨时间行动的本地 Agent：基于你当下的生活语境，用对话式、个性化的方式提醒和督促你。
...
traceflow 只做 Todo List 做不到的那部分：
...
存放那些应该跨对话留存、但还不想立刻变成硬任务的东西。只有两种类型：

- `wishseed`：未来想做的事、想试/想买的东西、想读想看的内容、存下来的链接、半成型的想法——任何你之后可能想回头看的。
- `concern`：还没拆解的担忧、风险、需要一直留在雷达上的重事。

用户一句话种草，系统负责收好；几天后模型在合适的时机翻出来，建议你回顾或执行。Reminder 管时机，seedbox 管保存——时间敏感的跟进归 reminder，跨天留存的归 seedbox。
...
- 在 pulse/安静回顾时，模型先 review pool，再决定每一条是删掉、提升成 reminder、还是提升成 seedbox。
...
1. `user_message`（主动对话）：先回答用户，再判断是否产生跟进锚点、习惯状态变更、seedbox 保存或长期记忆。
...
- **research**：演进中的问题、假设、来源笔记、临时观点，不是默认扫描，只在有活跃调查线程时才加载。
...
- **队列与触发**：本地提醒队列、pulse/check-in 触发队列、时间线截图任务队列，加上 seedbox / title-pool / memory 等内部存储。
...
模型不通过本地 CLI 调用能力，而是用项目原生的结构化工具。Claude Code 通过 workspace-local `.mcp.json`（启动时由 traceflow 注入并带 `--mcp-config`）发现这些工具；Codex 通过 runtime 侧的 Cyberboss MCP bridge 加载。

统一回顾与决策：
`cyberboss_pulse_review` · `cyberboss_followup_decide`

Pool（title pool）：
`cyberboss_title_pool_add` · `cyberboss_title_pool_list` · `cyberboss_title_pool_review` · `cyberboss_title_pool_remove` · `cyberboss_title_pool_promote_to_reminder` · `cyberboss_title_pool_promote_to_seedbox`

Memory：
`cyberboss_memory_remember` · `cyberboss_memory_search` · `cyberboss_memory_list` · `cyberboss_memory_update` · `cyberboss_memory_forget`

Research：
`cyberboss_research_upsert` · `cyberboss_research_search` · `cyberboss_research_list` · `cyberboss_research_archive`
...
Seedbox 数据模型说明：只有 `wishseed` 和 `concern` 两种 kind；活动项只暴露 `id` / `kind` / `title` / `tags` / `notes` / `createdAt` / `updatedAt`，完成后加 `completedAt`。不要把旧的 `status` / `priority` / `nextAction` 当作当前工具契约的一部分。
...
- `seedbox.json` — 种草/担忧留存
- `agent-memories.json` / `agent-research.json` — 记忆与调查
...
- **adapters/channel/**：收发消息、typing/media/context token，不碰 Codex/Claude 线程逻辑，也不碰 reminder/habit/seedbox/timeline/diary。
- **adapters/runtime/**：把消息送进具体 Agent runtime，处理线程/会话/批准/stop，注入模型侧指令层和项目原生工具。
- **services + habit**：reminder、habit、seedbox、title-pool、memory、research、diary、obsidian、timeline、sticker、vision 等本地行为/状态模块，由 app/runtime 流程协调，而非各自独立成产品。
- **integrations/**：timeline、whereabouts 等外部集成，尽量依赖独立项目而非折回主仓库。

## 与原项目的差异

相对 [WenXiaoWendy/Cyberboss](https://github.com/WenXiaoWendy/Cyberboss)，本发行版（cyberboss-traceflow）的重心从「全天候强外部监督的 ADHD Coach」收窄到「生活语境型轻度提醒」：保留 reminder / habit / timeline / diary / 微信桥接 / 双 runtime 等基础设施，但新增并强化了 seedbox（种草/担忧留存）、title-pool（上下文速记）、pulse 统一回顾、习惯建议引擎与最小可行版本、Obsidian 语境接入，并把默认闭合原则改成「未闭环就倾向于排提醒 / 存 pool，而不是嘴上记得」。它明确不竞争 Todo List，只补 Todo List 的盲区。
