...
- **Queue & Trigger System**
  本地队列与触发系统，当前包含 reminder、pulse/checkin system message、timeline screenshot 队列，以及用于保存未来有用线索的 seedbox。
- **Capability Layer**
  涵盖 Timeline、Diary、Pulse/Check-in、File Transfer 等核心能力，其中 `checkin` 是宿主侧命名，模型语义上对应 `pulse`。
- **Optional Tooling**
...
这里的 `checkin` 指的就是“随机轮询唤醒”能力，不是固定整点提醒；对模型来说，同一行为的语义名称是 `pulse`。
...
`checkin` 是宿主侧沿用的名称，本质上仍然是“随机轮询唤醒”能力。对模型来说，这套行为语义叫 `pulse`：系统会在一个随机时间点唤醒模型，让它自己判断现在该不该主动出现。
