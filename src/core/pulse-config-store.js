...
const DEFAULT_MIN_INTERVAL_MS = 180 * 60_000;
const DEFAULT_MAX_INTERVAL_MS = 360 * 60_000;

class PulseConfigStore {
  ...
  getRange(fallbackRange = resolveDefaultPulseRange()) {
    this.load();
...
function resolveDefaultPulseRange(env = process.env) {
  const minIntervalMs = readIntervalMs(env?.CYBERBOSS_PULSE_MIN_INTERVAL_MS, DEFAULT_MIN_INTERVAL_MS);
  const maxIntervalMs = Math.max(
    minIntervalMs,
    readIntervalMs(env?.CYBERBOSS_PULSE_MAX_INTERVAL_MS, DEFAULT_MAX_INTERVAL_MS)
  );
  return { minIntervalMs, maxIntervalMs };
}

function parsePulseRangeMinutes(input) {
...
function normalizeIntervalRange(value, fallbackRange = resolveDefaultPulseRange()) {
